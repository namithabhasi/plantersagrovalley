...
function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (location.search.includes('returnTo=payment')) {
        navigate('/payment');
      }
    } catch (error) {
    ...
    return (
      <div style={{marginTop:'40px'}}className="w-full space-y-6 text-left">
        <div>
                          ...
                          <div key={idx} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-100 pb-4 last:border-b-0 last:pb-0">
                            ...
                            <div className="flex items-start gap-4 flex-1 min-w-0">
                              ...
                              <div className="flex flex-col gap-1 flex-1 min-w-0 leading-snug">
                                ...
                                {/* Dynamic Amazon/Flipkart Lifecycle Subtext & 7-Day Policy Link */}
          ...
          {((ordersPagination?.totalPages || Math.ceil(orders.length / 4)) > 1) && (
            <div className="pagination-container !mt-8 !pt-6 border-t border-slate-100">
              ...
              {[...Array(ordersPagination?.totalPages || Math.ceil(orders.length / 4))].map((_, idx) => {
                const pageNum = idx + 1;
              ...
              <button
                onClick={() => setOrdersPage((prev) => Math.min(prev + 1, ordersPagination?.totalPages || Math.ceil(orders.length / 4)))}
                disabled={ordersPage === (ordersPagination?.totalPages || Math.ceil(orders.length / 4))}
                className="pagination-btn pagination-arrow"
                title="Next Page"
              >
